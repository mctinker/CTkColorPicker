...
class AskColor(customtkinter.CTkToplevel):
    def __init__(
        ...
        **button_kwargs,
        ...
        # Position the widget
        try:
            self.geometry(self.master.color_window_position)
        except (AttributeError, TypeError):
            self.geometry(f"{WIDTH}x{HEIGHT}")
        self.resizable(width=False, height=False)
        self.transient(self.master)
        self.lift()
        # Temporary fix, since a ReRun will not show a Color Picker window.
        self.deiconify()
    ...
    def _ok_event(self, event=None):
        self._color = self.label._fg_color
        self.master.color_window_position = self.wm_geometry()
        self.grab_release()
        self.destroy()
        del self.img1
        del self.img2
